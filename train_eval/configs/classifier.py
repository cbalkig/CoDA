...
class ClassifierConfig:
    def __init__(self, cfg: Section):
        ...
        self.hidden_dims: List[int] = cfg.get('hidden_dims')

        stage_configs: Dict[Optional[Stages], ClassifierPhaseConfig] = {
            None: ClassifierPhaseConfig(cfg.get("default"))
        }

        for stage in Stages:
            if cfg.get(stage.value) is not None:
                stage_configs[stage] = ClassifierPhaseConfig(cfg.get(stage.value))
    ...
    def optimizer_learning_rate(self, stage: Optional[Stages] = None) -> float:
        return self._optimizer_learning_rate[None] if (
            ...
            self._optimizer_learning_rate[stage]
