...
class DataTransforms(AbstractTransform):
    ...
    def _aug_block(self) -> T.Compose:
        ops = [
            ...
            T.RandomApply([T.GaussianBlur(kernel_size=9, sigma=(0.5, 2.0))], p=0.10),  # ↓ from 0.15

            T.RandomApply([T.RandomPosterize(bits=3)], p=0.05),  # ↓ from 0.10
            T.RandomApply([T.RandomSolarize(threshold=128)], p=0.05),  # ↓ from 0.10
